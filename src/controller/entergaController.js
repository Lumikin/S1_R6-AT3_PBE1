...
const entregaController = {
    ...
    adicionarEntregas: async (req, res) => {
        try {
            ...
            if (!Pedido || !Distancia || !Peso || !Acrescimo || !Taxa || !Desconto || !Tipo || !Status || !tipoEntrega === "normal" || !tipoEntrega === "urgente" || valorDistancia < 0 || valorPeso < 0) {
                return res.status(400).json({ message: 'Dados invalidos' })
            }

            const ConsultarKm = entregaModel.consultarPrecoKM(Pedido)
            if (ConsultarKm.length === 0) {
                throw new Error(`Nao foi possivel consultar o preco em Km de ${Pedido}`);
            }
            const valorKm = ConsultarKm * Distancia
            
            const ConsultarKg = entregaModel.consultarPrecoKG(Pedido)
            if (ConsultarKm.length === 0) {
                throw new Error(`Nao foi possivel consultar o preco em Km de ${Pedido}`);
            }
            const valorKg = ConsultarKg * Peso
